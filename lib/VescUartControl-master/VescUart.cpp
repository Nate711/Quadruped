...
bool ProcessReadPacket(uint8_t* message, bldcMeasure& values, int len) {
	...
	case COMM_GET_VALUES:
		...
		values.rotorPosition = 0.0;
		return true;
		break;
	case COMM_ROTOR_POSITION:
	  // janky memory storageSerialPrint
		values.rotorPosition = buffer_get_float32(message,100000.0,&ind);
		// values.rotorPosition = buffer_get_float32(message,1.0,&ind);
...
int VescUartCheckComm(bldcMeasure& values) {
	uint8_t payload[256];
	int lenPayload = ReceiveUartMessage(payload);
	//DEBUGSERIAL.println(lenPayload);
	if (lenPayload > 4) { // 55
		bool read = ProcessReadPacket(payload, values, lenPayload); //returns true if sucessful
		return read?1:-1;
	}
...
// https://github.com/vedderb/bldc-tool/blob/4151d86d31255bbb08afe33b126d52657039a4b0/packetinterface.cpp
void VescUartSetPosition(float pos) {
	int32_t index = 0;
	uint8_t payload[5];
...
// https://github.com/vedderb/bldc-tool/blob/4151d86d31255bbb08afe33b126d52657039a4b0/packetinterface.cpp
// https://github.com/vedderb/bldc-tool/blob/4151d86d31255bbb08afe33b126d52657039a4b0/packetinterface.cpp
// https://github.com/vedderb/bldc/blob/1655c11f043103352e33f1234fd32714e3ef47ff/commands.c
/*
...
void VescUartSetDetect(disp_pos_mode mode) {
	int32_t index = 0;
	uint8_t payload[2];

	payload[index++] = COMM_SET_DETECT ;
	payload[index++] = DISP_POS_MODE_ENCODER;
	PackSendPayload(payload, 2);
}
...
void SerialPrint(const bldcMeasure& values) {
	if(values.rotorPosition == 0.0) {
		...
		DEBUGSERIAL.print("rotorPosition: "); DEBUGSERIAL.println(values.rotorPosition);
	}
}
