...
// CUSTOM
// Checks serial for payloads and returns true if read
int VescUartCheckComm(bldcMeasure& values);

// CUSTOMMMMM
///Sends a command to VESC to control the motor position
///@param position as float with the degree position for the motor

void VescUartSetPosition(float pos);

// CUSTOMMMMM
// Sends command to VESC to tell it to send encoder data
